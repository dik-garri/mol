...
├── draft/
│   └── <slug>.md                      # Черновики и материалы в работе – вкладка «Драфт»
├── archive/
│   └── <slug>.md                      # Архив (старые/завершённые материалы) – вкладка «Архив»
...
- Структура каждого разбора:
  ...
  4. `Разделы` (## 1, 2, 3...) – разбор по блокам стихов:
     ...
     - ❓ **Вопросы в зал** – 3–5 вопросов для обсуждения в группе (обязательно в каждом разделе)
     - Подтемы (###) – культурный, исторический, лингвистический контекст
     - ### 🔖 **Запас для ведущего** – в конце каждого раздела: **Трудные вопросы (возражения)** с готовыми ответами + **Резервные вопросы** на случай затухшего обсуждения
  5. `Богословский итог` – ключевые выводы
  6. `Дополнительно` – углублённый материал (необязательно)
  7. `Практические выводы` – применение
  8. `Молитва` – благодарность → покаяние → просьба
...
- **Вкладки** «Проповеди» / «Луки» / «Драфт» / «Архив» (счётчик у каждой). «Драфт» (`draft/`) – черновики и материалы в работе; «Архив» (`archive/`) – старые/завершённые материалы (исследования, doc-разборы и т.п.)
- **Подкатегории в «Архив»** (sub-tabs): Все / Серии НЗ / Серии ВЗ / Свойства Бога / Праздники / Семинары / Темы. Категория хранится в frontmatter `category: "<key>"` (ключи: `series-nt`, `series-ot`, `attributes`, `feasts`, `seminars`, `themes`). Автокатегоризация: `python3 archive/categorize.py` (правила по slug в самом скрипте)
